...
internal sealed class AuditLogDtoProfile : Profile
{
    public AuditLogDtoProfile()
    {
        this.CreateMap<Source, Target>()
            ...
            .ForMember(target => target.EntityType, options => options.MapFrom(source => source.EntityType.ToString()))
            .ForMember(target => target.ContractNumber, options => options.MapFrom(source => source.ContractNumber))
